...
def main():
                                ...
                                if pkt.highest_layer != 'ARP':
                                    ...
                                    try:
                                        ...
                                        filewriter.writerow([pkt.highest_layer, transport_layer, ipcat, ip.dst,
                                                             ...
                                                             pkt.length, i / (time.time() - start_time), 0])
        ...
        def MLP(): #Primarily used for training either a new model or updating a previous model
            ...
            if load == 'y':
                ...
                mlp = MLPClassifier(hidden_layer_sizes=(5), activation='logistic', max_iter=1000, verbose=True, tol=0.00001) # Designates the setting of the model before training
                ...
                #max_iter = maximum amoung of iterations that the model will do
        ...
        def csv_interval_gather(cap): # creates/rewrites 'Live.csv' file with 30 second intervals- writes header row - goes through packets, writing a row to the csv for each packet
            ...
            with open ('LiveAnn.csv', 'w', newline='') as csvfile:
                filewriter = csv.writer(csvfile, delimiter=',' , quotechar='|', quoting=csv.QUOTE_MINIMAL)
                filewriter.writerow(['Highest Layer', 'Transport Layer', 'Source IP', 'Dest IP', 'Source Port', 'Dest Port','Packet Length', 'Packets/Time', 'target' ])
                                    ...
                                    if pkt.highest_layer != 'ARP':
                                        ...
                                        try:
                                            if ip.src not in allowed_IP:
                                                    ...
                                                    ipcat = 0
                                            filewriter.writerow([pkt.highest_layer, transport_layer, ipcat, ip.dst, pkt[pkt.transport_layer].srcport, pkt[pkt.transport_layer].dstport, pkt.length, i/(time.time() - start_time), ipv ])
                                            print ("Time: ", time.time() - start_time)
                                            i += 1
                                        except AttributeError:
                                            if ip.src not in allowed_IP:
                                                    ...
                                                    ipcat = 0
                                            filewriter.writerow([pkt.highest_layer, transport_layer, ipcat, ip.dst, 0, 0, pkt.length, i/(time.time() - start_time), ipv ])
                                            print ("Time: ", time.time() - start_time)
                                            i += 1
                                        ...
                                        if pkt.arp.src_proto_ipv4 not in allowed_IP:
                                                ...
                                                ipcat = 0
                                        arp = pkt.arp
                                        filewriter.writerow([pkt.highest_layer , transport_layer, ipcat, arp.dst_proto_ipv4, 0, 0, pkt.length, i/(time.time() - start_time), 0])
                                        print ("Time: ", time.time() - start_time)
                                        i += 1
                        except (UnboundLocalError, AttributeError) as e:
                                pass
