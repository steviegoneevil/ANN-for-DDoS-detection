...
def main():
        ...
        def MLP(): #Primarily used for training either a new model or updating a previous model
            ...
            if load == 'y':
                ...
                mlp = MLPClassifier(hidden_layer_sizes=(5,5), activation='logistic', max_iter=1000, verbose=True) # number of hidden layers = 1 layer of 10 nodes
            ...
            print()
            print("First 50 Predictions: ", "\n" ,mlp.predict(X_test)[0:50]) #Prints first 20 predictions
            print()
            print("First 50 Probabilities: ", "\n",mlp.predict_proba(X_test)[0:50])#Prints first 20 probabilities
            print()
                    ...
                    if (end - start < 30):                
                        if pkt.highest_layer != 'ARP':                
                            ...
                            if pkt.highest_layer != 'ARP':
                                ...
                                try:
                                    filewriter.writerow([i, pkt.highest_layer, transport_layer, ip.src, ip.dst, pkt[pkt.transport_layer].srcport, pkt[pkt.transport_layer].dstport, pkt.sniff_time, pkt.length, i/(time.time() - start_time), ipv ])
                                except AttributeError:
                                    filewriter.writerow([i, pkt.highest_layer, transport_layer, ip.src, ip.dst, 0, 0, pkt.sniff_time, pkt.length, i/(time.time() - start_time), ipv ])
                            
                            else:
                                arp = pkt.arp
                                filewriter.writerow([i, pkt.highest_layer , transport_layer, arp.src_proto_ipv4, arp.dst_proto_ipv4, 0, 0, pkt.sniff_time, pkt.length, i/(time.time() - start_time), 0])
                    else:
                        return
